...
def get_opt():
    ...
    parser.add_argument("--ifal", action='store_true')#是否保留手臂
    parser.add_argument("--noc", action='store_true')#对服装周围去除
    parser.add_argument("--useffl", action='store_true')#是否用ffl
    parser.add_argument("--ffl_w", type=float, default=1.0)
    ...
    parser.add_argument('--checkpoint_dir', type=str,
                        default='checkpoints_', help='save checkpoint infos')
...
def train_tom(opt, train_loader, model, board):
    ...
    #############################################ffl#193
    ffl = FFL(loss_weight=opt.ffl_w, alpha=1.0)  # initialize nn.Module class
    # criterion
    criterionL1 = nn.L1Loss()
    ...
    for step in range(opt.keep_step + opt.decay_step):
        ...
        shape = inputs['shape']
        #################################################################
        shapenocloth=inputs['shapenocloth']

        agnostic = inputs['agnostic'].cuda()
        #####################################################################
        agnosticnocloth=inputs['agnosticnocloth'].cuda()
        c = inputs['cloth'].cuda()
        cm = inputs['cloth_mask'].cuda()
        ...
        outputs = model(torch.cat([agnosticnocloth, c, cm], 1))  # CP-VTON+
        ...
        visuals = [[im_h, shape, im_pose],
                   ...
                   [p_rendered, p_tryon, im]]  # CP-VTON+
'''
        visuals = [[im_h, shapenocloth, im_pose],
                   [c, pcm*2-1, m_composite*2-1],
                   [p_rendered, p_tryon, im]]  # CP-VTON+
        ...
        ############################################################################ffl#展示
        loss_ffl = ffl(p_tryon, im)
        if opt.useffl:
            loss = loss_l1 + loss_vgg + loss_mask + loss_ffl
        else:
        ...
        if (step+1) % opt.display_count == 0:
            ...
            ###################################################################ffl#tenbo
            board.add_scalar('FFL', loss_ffl.item(), step+1)
            t = time.time() - iter_start_time
            ###################################################################ffl#print
            print('step: %8d, time: %.3f, loss: %.4f, l1: %.4f, vgg: %.4f, mask: %.4f, ffl: %.4f'
                  % (step+1, t, loss.item(), loss_l1.item(),
                     loss_vgg.item(), loss_mask.item(),loss_ffl.item()), flush=True)
